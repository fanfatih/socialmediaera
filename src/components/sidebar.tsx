...
export default function Sidebar() {
  ...
  // Mencegah error "Hydration" saat web pertama kali dimuat
  useEffect(() => {
    setIsMounted(true);
  }, []);

  const handleLogout = () => {
    // Menggunakan window.confirm agar lebih aman di Next.js
  ...
  // Layar kosong sementara sebelum Sidebar siap (mencegah kedip)
  if (!isMounted) return <aside className="w-64 h-screen sticky top-0 border-r border-gray-200 dark:border-gray-800 bg-white dark:bg-[#0a0a0a]" />;
  ...
  return (
    ...
    </aside> /* INI DIA TAG PENUTUP YANG TADI HILANG! */
  );
}
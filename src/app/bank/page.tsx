...
export default function BankKontenPage() {
  ...
  useEffect(() => {
    // Bersihkan interval lama jika ada (mencegah double polling & memory leak)
    if (pollingRef.current) {
      clearInterval(pollingRef.current);
    }

    // Jalankan polling HANYA jika isTelegramActive true DAN token sudah ada
    if (isTelegramActive && telegramToken) {
      fetchTelegramMessages(); // Tarik data seketika tanpa nunggu 5 detik pertama
      pollingRef.current = setInterval(fetchTelegramMessages, 5000);
    }

    // Cleanup saat komponen ditutup atau state berubah
    return () => { 
      if (pollingRef.current) clearInterval(pollingRef.current); 
    };
  }, [isTelegramActive, telegramToken]); // <- KUNCI FIX: Pantau perubahan state ini

  // --- PERBAIKAN 3: togglePolling yang lebih ringkas dan aman ---
  const togglePolling = async () => {
    if (isTelegramActive) {
      // Cukup ubah state, interval otomatis dimatikan oleh useEffect di atas
      ...
      // Cukup ubah state, interval otomatis dijalankan oleh useEffect di atas

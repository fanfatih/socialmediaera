...
export default function CalendarPage() {
  ...
  const openModalForNew = (dateStr: string) => {
    ...
    setPillar("Edukasi"); setLinkVideo(""); setRevisionNote(""); setIsModalOpen(true);
  };
  ...
  const openModalForEdit = (e: React.MouseEvent, item: Content) => {
    ...
    setLinkVideo(item.linkVideo || ""); setRevisionNote(item.revisionNote || ""); setIsModalOpen(true);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const dataPayload = { title, status, publishDate: selectedDate, platforms, pillar, referenceUrl, caption, copywriting, linkVideo };
    if (editId) updateContent(editId, dataPayload); else addContent(dataPayload);
  ...
  const getStatusStyle = (s: Status) => {
    switch (s) {
      ...
      case "Approved": case "Published": return "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800/50";
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#1a1a1a] w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-800 flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-5 border-b border-gray-100 dark:border-gray-800">
              <div>
                <h2 className="font-bold text-xl text-gray-900 dark:text-white">{editId ? "Detail Konten" : "Rencana Konten"}</h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select disabled={isLockedForKaryawan} value={status} onChange={(e) => setStatus(e.target.value as Status)} className="w-full p-2.5 bg-gray-50 dark:bg-[#121212] border border-gray-200 dark:border-gray-800 rounded-lg text-sm text-gray-900 dark:text-white outline-none cursor-pointer disabled:opacity-60">
                    ...
                    {role === "KARYAWAN" && (status === "Approved" || status === "Published" || status === "Rejected") && (<option value={status}>{status}</option>)}
                  </select>
                </div>
